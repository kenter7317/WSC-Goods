...
class GmailApiService(
    ...
    fun sendEmail(to: String, subject: String, bodyText: String): Boolean {
        // 유효한 서비스 계정 키가 있는지 확인
        if (!isValidServiceAccountKey()) {
            logger.warn("유효한 서비스 계정 키 파일이 없습니다. Mock 이메일 서비스를 사용합니다.")
            logger.warn("실제 Gmail API를 사용하려면 Google Cloud Console에서 서비스 계정을 생성하고 키 파일을 교체해주세요.")
            return mockEmailService.sendEmail(to, subject, bodyText)
        }
        ...
        return try {
            ...
            logger.error("Gmail API 이메일 발송 실패, Mock 서비스로 대체: ${e.message}")
            mockEmailService.sendEmail(to, subject, bodyText)
        }
    }
}
